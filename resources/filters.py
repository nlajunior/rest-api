...
query_with_identifier="SELECT id, duration, fhr_value, date_created, identifier, device_id FROM db.test WHERE IDENTIFIER=%s AND DATE_FORMAT(date_created, '%Y-%m-%d')=%s AND duration>=%s AND duration<=%s ORDER BY duration LIMIT %s OFFSET %s"

query_without_identifier="SELECT id, duration, fhr_value, date_created, identifier, device_id FROM db.test WHERE DATE_FORMAT(date_created, '%Y-%m-%d')=%s AND duration>=%s AND duration<=%s ORDER BY duration LIMIT %s OFFSET %s"
