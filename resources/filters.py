...
query_with_identifier="SELECT id, duration, fhr_value, date_created, identifier FROM db.test WHERE identifier=%s AND DATE_FORMAT(date_created, '%Y-%m-%d')=%s AND duration>=%s AND duration<=%s AND status=0 ORDER BY identifier, duration asc LIMIT %s OFFSET %s"
query_without_identifier="SELECT id, duration, fhr_value, date_created, identifier FROM db.test WHERE DATE_FORMAT(date_created, '%Y-%m-%d')=%s AND duration>=%s AND duration<=%s AND status=0 ORDER BY identifier, duration asc LIMIT %s OFFSET %s"
