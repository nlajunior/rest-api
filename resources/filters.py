...
def normalize_path_params(identifier=None, duration_min=1, duration_max=400, limit=20, offset=0, **data):
...
query_with_identifier="SELECT duration, fhr_value, date_created, identifier, device_id FROM db.test WHERE identifier=%s AND DATE_FORMAT(date_created, '%Y-%m-%d')=%s AND duration>=%s AND duration<=%s AND status=0 ORDER BY identifier, duration asc LIMIT %s OFFSET %s"
query_without_identifier="SELECT duration, fhr_value, date_created, identifier, device_id FROM db.test WHERE DATE_FORMAT(date_created, '%Y-%m-%d')=%s AND duration>=%s AND duration<=%s AND status=0 ORDER BY identifier, duration asc LIMIT %s OFFSET %s"
